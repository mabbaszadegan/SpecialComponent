...
function pageLoad() {
  ...
  controls.forEach(function (item) {
    if (
      item.getAttribute("placeholder") != null &&
    ...
    if (
      item.getAttribute("height") != null &&
      item.getAttribute("height") != ""
    )
      height = item.getAttribute("height");
    if (
      item.getAttribute("textPos") != null &&
      item.getAttribute("textPos") != ""
    )
    ...
    if (item.value.length > 0) animateText(item, "f");
    else animateText(item, "b");
